BDK_iRWR_GeneScore <- function(NewNetwork,prior_score,restart_p,damping,epsilon = 0.00000001,maxit=500){
  AdjMat = t(NewNetwork)
  ...
  restart_p_norm = restart_p/sum(restart_p)
  node_score_t = matrix(0,node_num_col,1)
  
  for(k in 1:maxit){
    
    node_score_t = damping*trans_Mat%*%node_score_norm + (1-damping)* restart_p_norm
